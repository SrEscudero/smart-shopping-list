...
import { useShoppingStore, CurrencySymbol, ListDensity } from '../../store/useShoppingStore';
import { ShoppingCart, Sun, Moon, Palette, ChevronRight, ChevronDown, Pencil, Check, X, DollarSign, LayoutList } from 'lucide-react';
...
const CURRENCY_OPTIONS: { id: CurrencySymbol; label: string }[] = [
  ...
  { id: '€', label: '€ Euro' },
  { id: '£', label: '£ Libra' },
  { id: 'ARS', label: 'ARS Peso AR' },
  { id: 'CLP', label: 'CLP Peso CL' },
  { id: 'COP', label: 'COP Peso CO' },
  { id: 'MXN', label: 'MXN Peso MX' },
  { id: 'PEN', label: 'PEN Sol' },
  { id: 'UYU', label: 'UYU Peso UY' },
];
...
export default function Dashboard() {
  const {
    ...
    toggleTheme, setAccentColor, toggleShoppingMode, setMonth, setCurrency, setListDensity
  } = useShoppingStore();
  ...
  useEffect(() => {
    if (editingMonth && monthInputRef.current) {
  ...
  const saveMonth = () => {
    if (monthValue.trim()) setMonth(monthValue.trim());
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {editingMonth ? (
              <div className="flex items-center gap-1.5 mb-1">
                ...
                />
                <button onClick={saveMonth} className="p-1 text-green-400 hover:bg-green-400/10 rounded-lg" style={{ minHeight: 'unset' }}>
                  <Check size={14} />
                </button>
                <button onClick={() => setEditingMonth(false)} className="p-1 text-red-400 hover:bg-red-400/10 rounded-lg" style={{ minHeight: 'unset' }}>
                  <X size={14} />
                </button>
              </div>
            ) : (
              <button
                onClick={startEditMonth}
                className="flex items-center gap-1.5 mb-1 group"
                style={{ minHeight: 'unset' }}
              >
                <p className="text-xs font-semibold uppercase tracking-widest opacity-50 font-display">
                  {month}
                </p>
      ...
      {/* ── CONTROLES DE TEMA Y PERSONALIZACIÓN ── */}
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)', background: 'var(--bg-card)' }}>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.div animate={{ rotate: showCurrencyPicker ? 180 : 0 }} transition={{ duration: 0.25 }}>
              <ChevronDown size={16} className="opacity-30" />
            </motion.div>
          </div>
        </button>
        ...
        <AnimatePresence>
          {showCurrencyPicker && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0 }}
              transition={{ duration: 0.25 }}
              className="px-4 pb-3 grid grid-cols-2 gap-1.5 overflow-hidden"
              style={{ borderBottom: '1px solid var(--border)' }}
            >
              {CURRENCY_OPTIONS.map((opt, i) => (
                <motion.button
                  ...
                  style={{
                    background: currency === opt.id ? 'var(--accent-soft)' : 'var(--bg-elevated)',
                    color: currency === opt.id ? 'var(--accent)' : 'var(--text-secondary)',
                    border: currency === opt.id ? '1px solid rgba(var(--accent-rgb), 0.3)' : '1px solid transparent',
                    minHeight: 'unset',
                  }}
                >
        ...
        {/* Densidad de lista */}
        <button
          onClick={() => setShowDensityPicker(!showDensityPicker)}
          className="w-full flex items-center gap-3 px-4 py-3.5 transition-all"
        >
          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-[var(--text-secondary)]" style={{ background: 'var(--bg-elevated)' }}>
            <LayoutList size={18} />
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.div animate={{ rotate: showDensityPicker ? 180 : 0 }} transition={{ duration: 0.25 }}>
              <ChevronDown size={16} className="opacity-30" />
            </motion.div>
          </div>
        </button>
            ...
            >
              {DENSITY_OPTIONS.map((opt, i) => (
                <motion.button
                  ...
                  style={{
                    background: listDensity === opt.id ? 'var(--accent-soft)' : 'var(--bg-elevated)',
                    color: listDensity === opt.id ? 'var(--accent)' : 'var(--text-secondary)',
                    border: listDensity === opt.id ? '1px solid rgba(var(--accent-rgb), 0.3)' : '1px solid transparent',
                    minHeight: 'unset',
                  }}
                >
                  <div>
                    <span className="text-sm font-semibold">{opt.label}</span>
                    <span className="text-[10px] ml-2 opacity-60">{opt.desc}</span>
                  </div>
